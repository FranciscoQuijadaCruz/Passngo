...
<head>
  ...
  <link rel="icon" type="image/png" href="imgs/favicon.ico" />
	<title>Pass 'N Go - Editar usuario.</title>
</head>
<body>
<?php
...
?>
<div>
<form action="edit_accounts_admin.php?userId=<?php echo $userDetails['id'];?>" method="post" class="margen">
<?php if(isset($error)) printMsg($error,"error"); ?>
    <div class="margeneditarcuentas"> 
     <h1 class="iniciosesion">Editar usuario.</h1>
		...
		<br>
  		...
  		<select name="rol" id="rol"  value="<?php echo $userDetails['rol']; ?>">
        
        <option value="agent" <?php if($userDetails['rol'] == "agent") echo 'selected="selected"'; ?>>agent</option>
        
        <option value="admin" <?php if($userDetails['rol'] == "admin") echo 'selected="selected"'; ?>>admin</option>
