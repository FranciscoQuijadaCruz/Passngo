...
if(isset($_GET['userId']) && is_numeric($_GET['userId'])) {
  ...
  $queryGetUserDetails = sprintf("SELECT * FROM evento WHERE id = %d",
  ...
  if(mysql_num_rows($resQueryGetUserDetails) == 0) header("Location: consulta_eventos.php?error=4");
...
if(isset($_POST['sent'])) {
  ...
  if(!isset($error)) {
    ...
    $queryAddEvento = sprintf("INSERT INTO papeleraevento (nombre, categoria, tipo, artistaOGrupo, fecha, hora, lugar, localidad, descripcion, imagen, numBoletos, precio, precioVip, latitud, longitud) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')",
...
if(isset($_POST['sent'])) {
  ...
  // Definir el query a ejecutar
  $queryUserDelete = sprintf("DELETE FROM evento WHERE id = %d",
    $_POST['id']
  );
  ...
  if($resQueryUserDelete) {
    header("Location: cpaneladmin.php?paperEvent=true");
  }
}
...
<head>
  ...
  <title>Pass 'N Go - Eliminar eventos.</title>
</head>
<body>
<?php
...
<div>
    ...
    <form action="enviar_pap_ev.php?userId=<?php echo $userDetails['id'];?>" method="post" class="margen">
    <div class="margeneditarcuentas">
        <h1 class="addemp">Enviar a Papelera</h1>
        ...
        <input for="id" type="hidden" name="id" value="<?php echo $userDetails['id']; ?>"/>
