...
if(isset($_GET['userId']) && is_numeric($_GET['userId'])) {
  ...
  header("Location: cpaneladmin.php?error=3");
} 
...
if(isset($_POST['sent'])) {
  ...
  $queryUserDelete = sprintf("DELETE FROM favoritos WHERE id = %d",
    $_POST['id']
  );
  ...
  if($resQueryUserDelete) {
    header("Location: cpaneladmin.php?deletedEvent=true");
  }
}
...
<div>
    ...
    <form action="delete_events.php?userId=<?php echo $userDetails['id'];?>" method="post" class="margen">
    <div class="margeneditarcuentas">
        <h1 class="addemp">Eliminar eventos</h1>
